...
while True:
    ...
    if event == "-OPEN-":
        path = os.getcwd()
        if os.path.exists(path + "\Sudokus"):
            os.startfile(path + "\Sudokus")
        else:
    ...
    if event == "-SUBMIT-":
        ...
        # c,d,e,f,g are all radio button variables         #change variable names
        ...
        if str(times).isdigit() is True:
            ...
            if int(times) == 0 or int(times) > 100:  # Exception handling for float numbers and values < 0 and > 100
                sg.popup("Input can not be < 1 or > 100.\nPlease enter a valid number", font=font,
                         title="Invalid Input")
                continue
        else:
